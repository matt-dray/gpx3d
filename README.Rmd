...
An in-development, opinionated package to create interactive 3D plots of workout routes.

Takes a .gpx file---downloaded from Apple Health for example---and extracts the time, coordinates and elevation into an sf-class object. You can then plot this as a 3D interactive object thanks to [{ggrgl}](https://coolbutuseless.github.io/package/ggrgl/index.html).
...
Use `plot_gpx_route()` to plot the object output from `extract_gpx_route()`. This opens in an external RGL device.
...
The chart is interactive so you can click and drag the chart, and scroll to zoom. here's a more exaggerated view of the climb from the south-west.
